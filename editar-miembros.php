...
<main>
<form action="actualizar.php" method="POST" class="row g-3" style="background-color: #F9E308; margin: 30px 25%; padding: 20px">
    <h3 style="text-align: center">Carga de miembros del club</h3>
    <div class="row g-3">
        <div class="col">
            <label for="inputNombre" class="form-label">Nombre</label>
            <input type="text" value="<?php echo $row['nombre'];?>" class="form-control" name="nombre" placeholder="Nombre">
        </div>
        <div class="col">
            <label for="inputApellido" class="form-label">Apellido</label>
            <input type="text" value="<?php echo $row['apellido'];?>" class="form-control" name="apellido" placeholder="Apellido">
        </div>
    </div>
    <div class="row g-3">
        <div class="col">
            <label for="inputDNI" class="form-label">DNI</label>
            <input type="number" value="<?php echo $row['id'];?>" class="form-control" name="id" placeholder="DNI">
        </div>
        <div class="col">
            <label for="inputDireccion" class="form-label">Dirección</label>
            <input type="text" value="<?php echo $row['direccion'];?>" class="form-control" name="direccion" placeholder="Dirección">
        </div>
    </div>
    <div class="col-md-2">
        <label for="edad" class="form-label">Edad</label>
        <input type="number" value="<?php echo $row['edad'];?>" class="form-control" name="edad">
    </div>
    <div class="col-md-4">
        ...
        <select name="cargo" class="form-select">
            ...
            for ($i=0;$i<=4;$i++){ 
              if ($cargos[$i]==$row['cargo']){ ?>
                <option selected value="<?php echo $cargos[$i];?>"><?php echo $cargos[$i];?></option>
              <?php } else {?>
    ...
    <div class="col-md-4">
        ...
        <select name="clase" class="form-select">
          ...
          for ($i=0;$i<=6;$i++){ 
            if ($clases[$i]==$row['clase']){ ?>
            <option selected value="<?php echo $clases[$i];?>"><?php echo $clases[$i];?></option>
            <?php } else {?>
